...
import { parseLicensePlate, getActiveQrs, removeActiveQr } from '../../auth/services/auth';
          ...
          if (allResp.data && Array.isArray(allResp.data)) {
            ...
            for (const s of mySessions) {
              ...
              if (!isCompleted && !isCancelled && sQrCode) {
                const { addActiveQr } = await import('../../auth/services/auth');
                addActiveQr(sQrCode);
              } else if ((isCompleted || isCancelled) && sQrCode) {
                const { removeActiveQr } = await import('../../auth/services/auth');
