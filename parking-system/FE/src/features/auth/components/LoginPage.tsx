...
import { syncCurrentUserFromApi } from '../services/auth';
...
const LoginPage = () => {
  ...
  const loginGoogle = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        setError(err.response?.data?.message || (language === 'en' ? 'Google Login failed.' : 'Đăng nhập Google thất bại.'));
      }
    },
    onError: () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || err.response?.data?.errors?.[0] || (language === 'en' ? 'Login failed. Please verify your credentials.' : 'Đăng nhập thất bại. Vui lòng kiểm tra lại thông tin.');
      ...
      <section className="hidden lg:flex flex-col justify-between w-[55%] p-20 relative z-10">
        ...
        <div className="max-w-2xl opacity-0 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
          <h2 className="text-[72px] font-display font-extrabold leading-[1.05] tracking-tight mb-8 text-on-surface drop-shadow-sm">
            {language === 'en' ? (
              <>Manage parking<br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-indigo-500 to-blue-500">smarter.</span></>
            ) : (
              <>Quản lý bãi xe<br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-indigo-500 to-blue-500">thông minh hơn.</span></>
            )}
          </h2>
          <p className="text-on-surface-variant text-xl leading-relaxed max-w-lg font-medium">
            {language === 'en'
              ? 'Operating system for the entire parking facility — from gate check-in to slot assignment, billing, and reports — in a single unified platform.'
        ...
        <div className="flex items-center space-x-16 opacity-0 animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
          <div className="flex flex-col">
            ...
            <span className="text-outline text-[10px] uppercase tracking-[0.2em] font-bold">
              {language === 'en' ? 'System Uptime' : 'Uptime hệ thống'}
            </span>
          </div>
        ...
        <div className="absolute bottom-40 right-20 glass-panel glow-border p-5 rounded-2xl flex items-center space-x-5 animate-float opacity-0 animate-fade-in-up" style={{ animationDelay: '0.6s' }}>
          ...
          <div>
            <p className="text-[9px] text-outline uppercase tracking-[0.25em] font-black">
              {language === 'en' ? 'Available Spots' : 'Slot khả dụng'}
            </p>
        ...
        <div className="w-full max-w-[460px] bg-white/90 border border-white/60 backdrop-blur-xl p-10 md:p-12 rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05),0_0_40px_rgba(37,99,235,0.05)] relative overflow-hidden group/card">
          ...
          <div className="mb-10 text-center">
            ...
            <p className="text-slate-500 font-medium text-sm">
              ...
              <Link to="/register" className="text-blue-600 hover:text-blue-700 transition-colors font-bold underline underline-offset-4 decoration-blue-500/30">
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 ml-1 block" htmlFor="password">
              ...
              >
                <span className="relative z-10 flex items-center justify-center gap-2">
                  {loading ? (language === 'en' ? 'PROCESSING...' : 'ĐANG XỬ LÝ...') : (language === 'en' ? 'Sign In' : 'Đăng nhập')}
              ...
              <div className="relative flex py-1 items-center">
                ...
                <span className="flex-shrink mx-4 text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400/80">
                  {language === 'en' ? 'Or sign in with' : 'Hoặc đăng nhập bằng'}
                </span>
                <div className="flex-grow border-t border-slate-200/40"></div>
              </div>
