...
const AdminBlacklist = () => {
  ...
  const handleResolveReport = async (report: any, action: 'approve' | 'reject') => {
    ...
    try {
      ...
      await adminService.resolveIncident(report.id);
      fetchBlacklist();
      fetchPendingReports();
    } catch (e) {
      console.error(e);
  ...
  const handleSendNotification = async (e: React.FormEvent) => {
    ...
    try {
      await adminService.pushNotification( {
        role: notifRole,
        title: notifTitle,
          ...
          <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8 animate-fade-in-up">
            ...
            <div className="mb-2">
              ...
              <p className="text-[13px] text-slate-500 mt-1.5 font-medium">
                {'Quản lý các phương tiện bị cấm và gửi thông báo hệ thống'}
              </p>
            </div>
                    ...
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <div>
                        <h2 className="text-lg font-black text-slate-800">
                          {`Báo cáo chờ duyệt (${pendingReports.length})`}
                        </h2>
                        <p className="text-xs font-bold text-amber-600">
                          {'Admin cần xem xét để đưa vào danh sách đen'}
                        </p>
                      </div>
                    </div>
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {details && (
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3 bg-slate-50 p-3 rounded-xl border border-slate-100 text-xs">
                                    ...
                                    <div>
                                      <span className="text-slate-400 font-bold uppercase tracking-wider block text-[9px] mb-0.5">
                                        {'Khu vực & Thời gian vào'}
                                      </span>
                                      <span className="font-semibold text-slate-700">{details.parkingLot || ('Không rõ')} <br/>{details.entryTime || '—'}</span>
                                    </div>
                                  </div>
                                )}
    
                                <p className="text-sm font-semibold text-slate-700">
                                  {'Lý do: '}<span className="font-medium text-slate-600">{displayReason}</span>
                                </p>
                            ...
                            <td colSpan={5} className="py-12 text-center">
                              <div className="flex flex-col items-center">
                                ...
                                <p className="text-sm font-bold text-slate-600">
                                  {'Không có phương tiện bị cấm'}
                                </p>
                                <p className="text-xs text-slate-400 mt-1">
                                  {'Hệ thống đang sạch, không có xe nào trong danh sách đen 🎉'}
                                </p>
                              </div>
                            </td>
                          </tr>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-[11px] text-slate-400 font-medium">
                          {`${notifHistory.length} thông báo đã gửi`}
                        </p>
                      </div>
                    </div>
